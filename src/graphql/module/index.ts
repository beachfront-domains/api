...
export function graphqlHTTP(options: Options): Middleware {
  ...
  return async function graphqlMiddleware(request: Request, response: Response): Promise<void> {
    console.log("—————");
    console.log(1);
    console.log("—————");
    ...
    try {
      ...
      if (schema === null)
        throw httpError(500, "GraphQL middleware options must contain a schema.");
...
function respondWithGraphiQL(response: Response, options?: GraphiQLOptions, params?: GraphQLParams, result?: FormattedExecutionResult): void {
  console.log("—————");
  console.log(2);
  console.log("—————");
...
export async function getGraphQLParams(request: Request): Promise<GraphQLParams> {
  ...
  let variables = (urlData.get("variables") ?? bodyData.variables) as { readonly [name: string]: unknown; } | null;
