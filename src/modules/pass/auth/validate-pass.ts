...
export default async(responseObject: any, hash: string, userEmail: string) => {
  const databaseConnection = await r.connect(databaseOptions);

  const passQuery = await r.table("passes").filter({ uid: userEmail })
  ...
  try {
    ...
    try {
      try {
        // Verify account
        await r.table("users").filter({ email: userEmail })
